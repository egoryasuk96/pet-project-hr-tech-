...
| **Запущенная заявка** | Справочный объект анализа: уже отправленная заявка со своим RouteInstance (не участник UI) |
...
1. Проверка роли `admin` (ACL-04 — backlog; не определён в Baseline rbac-matrix; см. [consistency-review.md](../../consistency-review.md) RR-ACL-01).
...
4. Сохранение не ретроактивно для RouteInstance уже отправленных заявок (BR-09).
...
2. Уже созданные / отправленные заявки продолжают согласование по своим RouteInstance.

### 3.4. Связь с runtime (без новых правил)

| Ситуация | Поведение |
| :--- | :--- |
| Admin меняет маршрут после submit заявки R | R продолжает по своему RouteInstance (AC-APP-10) |
...
sequenceDiagram
  ...
  Note over Req: Уже есть RouteInstance<br/>(создан при первом submit)
  ...
  alt Не admin
    ...
    Sys->>Sys: SnapshotIsolation: не менять RouteInstance Req (BR-09)
...
| **RBAC/ACL** | ACL-04, ACL-09 — backlog; не определены в Baseline rbac-matrix ([consistency-review.md](../../consistency-review.md) RR-ACL-01) |
| **BPMN** | BPMN-03 |
