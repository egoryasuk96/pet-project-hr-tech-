...
| ST-06 | Система | Зафиксировать изоляцию RouteInstance / FieldValueVersion | Аннотируемое следствие: изменения **не** меняют RouteInstance уже отправленных заявок (BR-09); FieldValueVersion уже отправленных заявок не переписываются конфигурацией (BR-26 для in-flight — данные зафиксированы на submit) |
...
| :--- | :--- | :--- |
| RouteInstance | Создаётся при первом submit заявки; конфиг после этого не меняет RouteInstance запущенной заявки (BR-08, BR-09) | BPMN-01 ST-04; AC-APP-10 |
...
3. Сохранение конфигурации не ретроактивно для RouteInstance уже отправленных заявок.
