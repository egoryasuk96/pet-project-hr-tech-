...
| ST-05 | Система | Определить следующий этап по RouteInstance | Чтение **существующего** RouteInstance заявки (не пересоздание) |
| ST-06 | Система | Создать задачи следующего этапа | Назначения из RouteInstance (BR-02, FR-APP-06) |
...
| GW-04 | Система | Есть следующий этап в RouteInstance? | Да → ST-06 → EE-01; Нет → ST-07 → EE-02 |
...
| SF-16 | ST-04 → ST-05 | Определить next step по **RouteInstance** |
| SF-17 | ST-05 → GW-04 | — |
...
4. **Approve:** опциональный комментарий → задача completed → прочие активные задачи этапа `cancelled` (BR-03) → по **RouteInstance** либо задачи следующего этапа, либо `approved` (BR-17).
5. **Reject:** обязательный комментарий → `rejected`, маршрут завершён (BR-04).
6. **Return:** обязательный комментарий → `returned`, номер этапа сохранён, задачи этапа закрыты (BR-05); дальнейший resubmit — в BPMN-01 с тем же RouteInstance.

RouteInstance в этом процессе **только читается**, не создаётся и не пересоздаётся.
