...
| ST-05 | Система | Определить следующий live-этап | По ApprovalStage.sequence_no от current_stage_id (BR-02) |
| ST-06 | Система | Создать задачи следующего этапа | Из live StageAssignment (BR-02, FR-APP-06) |
...
| GW-04 | Система | Есть следующий live-этап? | Да → ST-06 → EE-01; Нет → ST-07 → EE-02 |
...
| SF-16 | ST-04 → ST-05 | Определить next live ApprovalStage |
| SF-17 | ST-05 → GW-04 | — |
...
4. **Approve:** опциональный комментарий → задача completed → прочие активные задачи этапа `cancelled` (BR-03) → по **live** ApprovalStage либо задачи следующего этапа, либо `approved` (BR-17).
5. **Reject:** обязательный комментарий → `rejected`, маршрут завершён (BR-04).
6. **Return:** обязательный комментарий → `returned`, `current_stage_id` сохранён, задачи этапа закрыты (BR-05); дальнейший resubmit — в BPMN-01 с первого live-этапа.

Следующий этап определяется по **live** ApprovalRoute, не snapshot.
