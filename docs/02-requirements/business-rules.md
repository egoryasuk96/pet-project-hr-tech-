...
После `return` инициатор может изменить значения полей заявки (в статусе `returned`) и повторно отправить её на согласование. Повторный submit возобновляет согласование **с того же этапа**, на котором был выполнен return (по экземпляру маршрута).  
> **Open Question:** продолжать с того же этапа (как здесь) или начинать с первого — см. OQ-A / OQ-B в [Snapshot Model](../03-diagrams/erd/snapshot-model.md) §6. До выбора действует формулировка выше.  
...
| ID | Вопрос | Где |
| :--- | :--- | :--- |
| OQ-A / OQ-B | После return+resubmit: тот же этап или с первого? | [Snapshot Model §6](../03-diagrams/erd/snapshot-model.md); указатель в BR-06 |
