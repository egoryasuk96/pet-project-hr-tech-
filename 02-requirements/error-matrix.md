...
| ERR_ROUTE_CONFIG | 409 | Маршрут типа невалиден при submit: нет этапов и/или нет назначений (BR-18) | Маршрут согласования настроен некорректно. Обратитесь к администратору | Submit отклоняется; draft/returned сохраняется |
| ERR_CONFLICT_VERSION | 409 | **TBD:** конфликт параллельного изменения одной сущности (опционально для админки) | Данные были изменены другим пользователем. Обновите страницу | Изменение не применяется |
...
| :--- | :--- | :--- |
| OQ-ERR-01 | 403 или 404 на чужую заявку | 404 + ERR_NOT_FOUND |
| OQ-ERR-02 | Нужен ли optimistic locking в админке | Отложить; ERR_CONFLICT_VERSION TBD |
